...
| `on_trip` | Executing an accepted trip. Home-summary carries the full `currentTrip` block (current stop, progress, on-board list, running earnings). Returns to `active` when the trip completes or is cancelled. |
